...
./b2 install toolset=${TOOLCHAIN}-arm target-os=android -j ${CK_HOST_CPU_NUMBER_OF_PROCESSORS} link=static address-model=${CK_TARGET_CPU_BITS} --prefix=${BOOST_BUILD_PATH} ${BOOST_B2_FLAGS}
