...
def do(i, arg):
    ...
    ii={'action':'load',
        'module_uoa':'program',
        'data_uoa':'acl-sgemm-opencl-example'}
    rx=ck.access(ii)
    ...
    # CLblast libs.
    depl=copy.deepcopy(cdeps['lib-arm-compute'])
    #ON LOCAL MACHINE
    ...
    'device_id':tdid,
    'out':'con',
    'deps':{'lib-arm-compute':copy.deepcopy(depl)}
    }
    r=ck.access(ii)
    if r['return']>0: return r
    udepl=r['deps']['lib-arm-compute'].get('choices',[])
    if len(udepl)==0: return {'return':1, 'error':'not installed lib-arm-compute'}
    ...
    record_repo='local'
    record_uoa='explore-matrix-size-'+'acl-sgemm-opencl'
    ck.out('---------------------------------------------------------------------------------------')
    ...
    ii={
        ...
        'tags':['explore-clblast-matrix-size-client', 'acl-sgemm-opencl', size_tag, platform_id],
        'pipeline': cpipeline,
        'out':'con'
